...
LOUDNESS_MIN_LAEQ = 45.0
LOUDNESS_INTERVAL = 20
LOUDNESS_KEEPALIVE = 60
...
def main():
    ...
    fifo_fd = None
    fifo_path = cfg.get("fifo_path", "/tmp/saem_loudness_fifo")

    try:
        fifo_fd = os.open(fifo_path, os.O_WRONLY)
        print(f"[NICU] FIFO connected (blocking): {fifo_path}")
    except Exception as e:
        print(f"[NICU] FIFO not available ({e}), continuing without perceptual stream")
        fifo_fd = None
    ...
    try:
        while RUNNING:
            ...
            if band_bank is not None:
                ...
                compute_third = (
                    row_count % int(cfg.get("third_octave_interval_s", 60)) == 0
                )
            ...
            if ctx_cfg.get("enabled", False) and np.isfinite(laeq_bg_eff):
                ...
                if trigger:
                    ...
                    # Filtro ligero para impulsive (como antes)
                    if label == "impulsive":
            ...
            # ================================
            # LOUDNESS FIFO SEND (ULTRA OPTIMIZED)
            # ================================
            send_loudness = False

            # Active mode: cada 25 s si hay actividad
            if (row_count % LOUDNESS_INTERVAL == 0) and (laeq_1s_dbA >= LOUDNESS_MIN_LAEQ):
                send_loudness = True

            # Keepalive: cada 60 s aunque esté silencioso
            if (row_count % LOUDNESS_KEEPALIVE == 0):
                send_loudness = True

            if fifo_fd is not None and send_loudness:
                try:
                    chunk = np.asarray(x, dtype=np.float32)
                    os.write(fifo_fd, chunk.tobytes())

                except BrokenPipeError:
                    print("[NICU] FIFO broken pipe")

                except Exception as e:
                    if cfg.get("verbose", False):
                        print(f"[NICU][FIFO ERROR] {e}")

...
def main():
    ...
    fieldnames += ["trigger", "label", "confidence"]
    ...
    try:
        while RUNNING:
            ...
            laeq_bg = leq_900.value_db() if leq_900.is_full() else np.nan
            ...
            trigger = False
            if ctx_cfg.get("enabled", False) and leq_900.is_full():
                trigger = laeq_1s_dbA > (laeq_bg + float(ctx_cfg["trigger_delta_db"]))
            ...
            row["label"] = label
            row["confidence"] = "" if np.isnan(conf) else round(conf, 3)
            ...
            bg_txt = "..." if np.isnan(laeq_bg) else f"{laeq_bg:.2f}"

...
def main():
    ...
    try:
        while RUNNING:
            ...
            # ================================
            # LOUDNESS FIFO SEND (FINAL OPTIMIZED)
            # ================================
            send_loudness = False
            ...
            if fifo_fd is not None and send_loudness:
                try:
                    chunk = x.astype(np.float32).tobytes()

                    # no bloqueante (sin timeout innecesario)
                    _, wlist, _ = select.select([], [fifo_fd], [], 0)
